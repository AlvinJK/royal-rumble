...
export const writeFileContents = (fileName, content) => {
  try {
    ...
    console.log(`${fileName} generated successfully, please check`);
  } catch (err) {
    console.error(err);
  }
};
...
// Sorting romans function
const sortRomans = (romanA, romanB) => {
...
export const getSortedList = (romans) => {
  ...
  return romanClass.sort(sortRomans).map((roman) => roman.name);
};
